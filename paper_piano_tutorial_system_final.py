...
CORRECT_COLOR = (40, 220, 40)
WRONG_COLOR = (0, 0, 220)
GUIDE_FLASH_SEC = 0.35
...
class TutorialState:
    ...
    def register_press(self, key_idx: int, synth: "SimpleSynth", now: float) -> None:
        ...
        if key_idx == target:
            self.correct_flash_until[key_idx] = now + GUIDE_FLASH_SEC
            self.step += 1
            if self.step >= len(SONG_SEQUENCE):
                ...
                print(
                    f"[Tutorial] CORRECT {NOTE_NAMES[key_idx]} "
                    f"{self.step}/{len(SONG_SEQUENCE)} errors={self.errors}"
                )
        else:
            self.wrong_flash_until[key_idx] = now + GUIDE_FLASH_SEC
            self.errors += 1
            print(
                f"[Tutorial] WRONG {NOTE_NAMES[key_idx]} "
...
def draw_tutorial_key_guide(
    ...
    if H_inv is None or not ts.mode:
        return
    ...
    for idx, (x0, y0, x1, y1) in enumerate(mapper.build_key_rects()):
        ...
        if flip_code is not None:
            poly_img = np.array([_transform_display_point(pt, frame.shape, flip_code) for pt in poly_img], dtype=np.float32)
        ...
        if now < ts.correct_flash_until[idx]:
            ...
            thickness = 4
            alpha = 0.35
        elif now < ts.wrong_flash_until[idx]:
            color = WRONG_COLOR
            thickness = 4
            alpha = 0.40
        elif idx == ts.target_key:
            color = GUIDE_COLOR
            thickness = 4
            alpha = 0.28
        ...
        _fill_tutorial_poly(frame, poly_img, color, alpha)
        cv2.polylines(frame, [np.int32(poly_img)], True, color, thickness)
...
def draw_tutorial_hud(frame: np.ndarray, ts: TutorialState) -> None:
    ...
    h, w = frame.shape[:2]
    hud_x, hud_y, hud_w, hud_h = w - 422, 14, 408, 162
    overlay = frame.copy()
    cv2.rectangle(overlay, (hud_x - 6, hud_y), (hud_x + hud_w, hud_y + hud_h), (20, 20, 20), -1)
    cv2.addWeighted(overlay, 0.60, frame, 0.40, 0, frame)
    cv2.rectangle(frame, (hud_x - 6, hud_y), (hud_x + hud_w, hud_y + hud_h), (100, 100, 100), 1)
    font = cv2.FONT_HERSHEY_SIMPLEX

    if ts.completed:
        cv2.putText(frame, "Song Complete!", (hud_x, hud_y + 38), font, 1.0, (40, 220, 40), 2)
        cv2.putText(frame, f"Errors: {ts.errors}", (hud_x, hud_y + 72), font, 0.75, (220, 220, 220), 2)
        cv2.putText(frame, "R=restart   T=free-play", (hud_x, hud_y + 110), font, 0.62, (160, 160, 160), 1)
        return

    total = len(SONG_SEQUENCE)
    step = ts.step
    target = ts.target_key
    cv2.putText(frame, SONG_NAME, (hud_x, hud_y + 28), font, 0.62, (220, 200, 80), 2)
    if target is not None:
        cv2.putText(frame, "Press:", (hud_x, hud_y + 62), font, 0.65, (200, 200, 200), 1)
        cv2.putText(frame, NOTE_NAMES[target], (hud_x + 82, hud_y + 62), font, 1.0, (255, 200, 0), 3)
        upcoming = [NOTE_NAMES[SONG_SEQUENCE[s]] for s in range(step + 1, min(step + 5, total))]
        cv2.putText(frame, "  ->  ".join(upcoming), (hud_x, hud_y + 92), font, 0.55, (130, 130, 130), 1)

    bar_x, bar_y, bar_w, bar_h = hud_x, hud_y + 108, hud_w - 12, 14
    cv2.rectangle(frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), (60, 60, 60), -1)
    cv2.rectangle(frame, (bar_x, bar_y), (bar_x + int(bar_w * step / max(total, 1)), bar_y + bar_h), (40, 190, 40), -1)
    cv2.putText(frame, f"{step}/{total}", (bar_x + bar_w + 4, bar_y + 12), font, 0.5, (170, 170, 170), 1)
    error_color = (60, 60, 220) if ts.errors > 0 else (130, 130, 130)
...
def run_dual_camera_mode(top_cam: int, side_cam: int) -> None:
    ...
    print(f"\n=== Tutorial Mode (Dual Camera): {SONG_NAME} ===")
    ...
    print("Controls: q quit | r restart tutorial | t toggle tutorial/free-play\n")
    ...
    try:
        while True:
            try:
                    ...
                    for obs in tracker_side.iter_finger_observations(bottom_frame, res_bottom):
                        ...
                        if is_pressed and not state.is_down:
                            ...
                            if key_idx is not None and (now - state.last_press_t) >= KEY_COOLDOWN_SEC:
                                ...
                                tutorial.register_press(key_idx, synth, now)
                ...
                draw_tutorial_key_guide(top_display, mapper, H_inv, tutorial, now, flip_code=-1)
                draw_tutorial_hud(top_display, tutorial)
                cv2.putText(top_display, f"Tutorial: {'ON' if tutorial.mode else 'OFF (free-play)'}",
                           (10, 205), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 2)
                ...
                if k == 27 or k == ord("q"):
                    ...
                    break
                if k in (ord("t"), ord("T")):
                    tutorial.mode = not tutorial.mode
                    print(f"[INFO] Tutorial {'ON' if tutorial.mode else 'OFF (free-play)'}")
                if k in (ord("r"), ord("R")):
                    tutorial.reset()
                    side_selector.reset()
...
def run_single_camera_mode(cam_idx: int) -> None:
    ...
    print(f"\n=== Tutorial Mode (Single Camera): {SONG_NAME} ===")
    ...
    try:
        while True:
            try:
                ...
                draw_tutorial_key_guide(proc_frame, mapper, H_inv, tutorial, now)
                draw_tutorial_hud(proc_frame, tutorial)
                cv2.putText(proc_frame, f"Tutorial: {'ON' if tutorial.mode else 'OFF (free-play)'}",
                           (10, 260), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 2)
